...
url_base = 'https://www.robotevents.com/api/v2/events?'
url_get = 'start=6%2F1%2F2022&end=5%2F29%2F2023&region=Tennessee&myEvents=false&eventTypes%5B%5D=tournament'
...
while i < maxEvent + 1:
    ...
    for event in eventData:
        if "VRC" in str(event['sku']):
            print(str(event['id']) + "  " + event['name'] + " " + str(event['start']))
    response = requests.get(url, headers=headers)
    json_data = json.loads(response.text)
    eventData =json_data['data']
    metaData=json_data['meta']
