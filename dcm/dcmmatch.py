...
def get_scores():
    url_base = 'https://www.robotevents.com/api/v2/events/49053/divisions/1/matches?'
    url_get = ''
    avgWin  =0 
    myWinPoints =0 
    myWonMatches =0
    ...
    while i < maxPage:
        ...
        for m in matchData:
            ...
            for adata in m['alliances']:
                ...
                #print(adata['color'])
                scored = ''
                if adata['score']: scored= "SCORED Q"
                else: scored = m['field'] + " Q"
                if adata['color'] == "blue": bluescore= adata['score']
                if adata['color'] == "red": redscore= adata['score']
                for myteams in adata['teams']:
                    if str('57711') in myteams['team']['name']:
                        myColor = adata['color']

                        print(scored + str(m['matchnum']) + '  ' +myColor + '  ' +  myteams['team']['name'] + ' @ ' + m['scheduled'])
            if myColor == 'red':
