...
def get_tasks(session,team):
    ids = get_team_events(team)
    team_id = get_team_details(team)[0]
    tasks = []
    for id in ids:
        team_matches_url = baseurl+"teams/"+str(team_id)+"/matches?event[]="+str(id)
        tasks.append(session.get(team_matches_url, headers=headers, data=payload))
    return tasks

async def get_matches_data(team):
    all_event_matches = []
    async with aiohttp.ClientSession() as session:
        tasks = get_tasks(session,team)
        responses = await asyncio.gather(*tasks)
        for response in responses:
            all_event_matches.append(await response.json())
        all_team_score = []
        all_opp_score = []
        matches_result = []
        per_match_info = []
        ...
        """for id in ids:
            team_matches_url = baseurl+"teams/"+str(team_id)+"/matches?event[]="+str(id)
            team_matches_response = await session.request("GET", team_matches_url, headers=headers, data=payload)
            all_event_matches.append(await team_matches_response.json())
        print(all_event_matches)"""

#get_matches_data(team)

print(asyncio.run(get_matches_data(team)))
